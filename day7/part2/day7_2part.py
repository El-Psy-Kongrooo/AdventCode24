def readFile():
    equations = {}
    with open("day7_2part.txt", "r") as file:
        for line in file:
            if line.strip():
...
def add(key, values, count):
    ...
    if len(values) == 0:
        return False
    return (add(key, values[1:], count + values[0]) or mul(key, values[1:], count + values[0]) or concat(key, values[1:], count + values[0]))
...
def concat(key, values, count):
    if key == count and len(values) == 0:
        return True
    if len(values) == 0:
        return False
    return (add(key, values[1:], int(str(count) + str(values[0]))) or mul(key, values[1:], int(str(count) + str(values[0]))) or concat(key, values[1:], int(str(count) + str(values[0]))))

def getNumbers(equations):
    listOfValues = []
    for key, equations_list in equations.items():
        for values in equations_list:
            if add(key, values[1:], values[0]) or mul(key, values[1:], values[0]) or concat(key, values[1:], values[0]):
